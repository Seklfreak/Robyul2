...
func getCurrentSinglePlayerGameByUserID(userID string) *singleBiasGame {
	if userID == "" {
		return nil
	}
	currentSinglePlayerGamesMutex.RLock()
	defer currentSinglePlayerGamesMutex.RUnlock()
	if game, ok := currentSinglePlayerGames[userID]; ok {
		return game
	}
	return nil
}

// sets a single player game
func setCurrentSinglePlayerGame(singlePlayerGame *singleBiasGame) {
	if singlePlayerGame == nil || singlePlayerGame.User.ID == "" {
		return
	}
	currentSinglePlayerGamesMutex.Lock()
	defer currentSinglePlayerGamesMutex.Unlock()
	currentSinglePlayerGames[singlePlayerGame.User.ID] = singlePlayerGame
}

// deletes a single player game by UserID
func deleteCurrentSinglePlayerGameByUserID(userID string) {
	if userID == "" {
		return
	}
	currentSinglePlayerGamesMutex.Lock()
	defer currentSinglePlayerGamesMutex.Unlock()
	if _, ok := currentSinglePlayerGames[userID]; ok {
...
// adds a new multi player game to the ongoing multi player games
func addCurrentMultiPlayerGame(multiPlayerGame *multiBiasGame) {
	currentMultiPlayerGamesMutex.Lock()
	defer currentMultiPlayerGamesMutex.Unlock()
	currentMultiPlayerGames = append(currentMultiPlayerGames, multiPlayerGame)
}

// deletes a multi player game by CurrentRoundMessageID
func deleteCurrentMultiPlayerGameByCurrentRoundMessageID(currentRoundMessageID string) {
	currentMultiPlayerGamesMutex.Lock()
	defer currentMultiPlayerGamesMutex.Unlock()
	for i, game := range currentMultiPlayerGames {
		if game.CurrentRoundMessageId == currentRoundMessageID {
