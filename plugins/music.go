...
func (m *Music) processorLoop() {
	...
	for {
		...
		for _, song := range songs {
			...
			// Create file
			opusFile, err := os.Create(helpers.GetConfig().Path("cache_folder").Data().(string) + name + ".ro")
			helpers.Relax(err)
			...
			cat := exec.Command(
				"ffmpeg",
				"-i", helpers.GetConfig().Path("cache_folder").Data().(string)+name+".wav",
				"-f", "s16le",
				"-ar", "48000",
				"-ac", "2",
				"-v", "128",
			...
			// Cleanup
			helpers.Relax(os.Remove(helpers.GetConfig().Path("cache_folder").Data().(string) + name + ".wav"))

			// Mark as processed
			song.Processed = true
			song.Path = helpers.GetConfig().Path("cache_folder").Data().(string) + name + ".ro"
...
func (m *Music) janitor() {
	...
	for {
		...
		dir, err := ioutil.ReadDir(helpers.GetConfig().Path("cache_folder").Data().(string))
		helpers.Relax(err)
		...
		for _, file := range dir {
			...
			for _, song := range songs {
				if strings.Contains(helpers.GetConfig().Path("cache_folder").Data().(string)+file.Name(), helpers.BtoA(song.URL)) {
					foundFile = true
					break
				}
			}
			...
			if !foundFile {
				...
				err = os.Remove(helpers.GetConfig().Path("cache_folder").Data().(string) + file.Name())
				helpers.Relax(err)
			}
		}
